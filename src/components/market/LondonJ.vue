...
<div class="londonJ">
	...
	<div class="container">	
		<p class="h2">	操作建议 </p>
		<p class="conDesc mt30">伦敦金支撑阻力	</p>
		<p class="conDesc">上方阻力：{{operation.llgzlw}}<br>下方支撑：{{operation.llgzcw}}	</p> 
	</div>
</div> 
</template>
	...
	methods:{ 
		getDatas(){ 
			this.$http.get('/GetScript/getGraphs')  
				.then((res)=>{      
					this.date = res.LLG.pop().time;
					this.data = res.MRHQ.QLLG; 
				}) 
			this.$http.get("/Mobile/Index/czjy")  //伦敦金多头空头
	  			.then((res)=>{    
	  				this.more = res.data.llgkd;  
	  				this.less = res.data.llgkk;  
